...
この記事では、**6つ全てを実際にZenn記事執筆リポジトリに導入しながら**、その違いと使い方を解説します。
...
| **Plugins** | 機能セット | - | チーム共有 | Skills/Hooksをまとめて共有したい場合 | △ 独自のPlugin機能あり |
...
Instructionsとの最大の違いは「**特定タスクの手順書として呼ばれた時だけ有効**」な点です。`SKILL.md` の `description` を読んで、関連する依頼が来たときに自動発動します。

### 特徴

- ✅ AIが `description` を見て自動発動
...
1. 「新規記事作りたい」と依頼
2. `/new-article` Skillが自動発動
3. 自動的にブランチ作成・ファイル生成

### 記事レビュー

1. 「記事をレビューして」と依頼
2. `/article-review` Skillが自動発動
3. チェックリストに沿ってレビュー実行
...
`.github/plugins/` の中の Skills（`.github/skills/`）は両AIで共有できます。

---

## 6. Cookbook Recipes（サンプル集）
...
6つ全てを導入した最終的なディレクトリ構成：

```text
.github/
├── copilot-instructions.md       # 1. Instructions（Copilot用）
├── skills/                        # 2. Skills（Copilot・Claude Code 共通）
│   └── zenn-article-lint/
│       ├── SKILL.md
│       └── references/
│           └── common-errors.md
├── hooks/                         # 4. Hooks（Git hooks・AI非依存）
│   ├── pre-commit
│   └── README.md
├── plugins/                       # 5. Plugins（Copilot用）
│   └── zenn-workflow/
│       ├── README.md
│       ├── skills/ → symlink
│       └── hooks/ → symlink
└── recipes/                       # 6. Recipes（AI非依存・共通）
    └── zenn-cli.md

CLAUDE.md                          # 1. Instructions（Claude Code用）

.claude/
├── skills/                        # 2. Skills（Claude Code専用追加分）
│   └── new-article/
│       └── SKILL.md
└── settings.json                  # 4. Hooks（Claude Code固有）

# 3. Agents は将来的に追加（Copilot: MCP対応後 / Claude Code: context:fork で代替可）
```
...
2. Skills        # 定型作業・複雑なタスク自動化
3. Hooks         # 品質保証
4. Plugins       # 全てをセットで共有
5. Recipes       # オンボーディング資料
```
...
この記事では、GitHub Copilotの6つの拡張機能を実際にZenn記事執筆リポジトリに導入しながら、Claude Codeでの対応状況も合わせて解説しました。

### 6つの違い おさらい

1. **Instructions** - AIが常に従うルール
2. **Skills** - 定型作業・複雑なタスクの自動化パッケージ
3. **Agents** - 自律的に動くアシスタント
4. **Hooks** - イベント駆動の自動実行
5. **Plugins** - 機能をまとめたスターターキット
6. **Recipes** - すぐ使えるサンプルコード集
