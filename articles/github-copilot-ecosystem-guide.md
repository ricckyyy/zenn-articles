...
加えて、Claude Code固有の **Hooks機能**（ツールイベントに反応するHooks）も別途存在します。こちらはAIのツール実行（ファイル読み書き、コマンド実行など）をトリガーにできます。ユーザー設定の `~/.claude/settings.json` またはプロジェクトの `.claude/settings.json` に記述します。
...
`.github/plugins/` の中の Skills（`.github/skills/`）は共有できますが、Prompts部分はClaude Code用に `.claude/skills/` へ移植が必要です。
