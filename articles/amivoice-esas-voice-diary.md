...
### 1. APIキーはクレカ登録後でないと発行されない

<!-- markdownlint-disable MD013 -->
AmiVoiceのAPIキーは、マイページでクレジットカードを登録してはじめて発行されます。無料枠内で使う場合も登録が必要です。発行場所は **マイページ → ダッシュボード → 接続情報** です。
<!-- markdownlint-enable MD013 -->

### 2. `$API_KEY` はクォートで囲む

ドキュメントのサンプルは `-F u=$API_KEY` ですが、APIキーに記号（`+` や `/` など）が含まれる場合にシェルが誤解釈してリクエストが失敗します。
...
### 3. パラメータ名は自分で取得する必要がある

感情解析のパラメータ名と最大値は、専用エンドポイントから取得します。ここを確認せずに実装するとスコアが全部0になります。
